...
Wolfringo Commands System aims to reduce the amount of boilerplate code, and that includes code required for such checks. For that reason, [Wolfringo.Commands](https://www.nuget.org/packages/Wolfringo.Commands) includes some of the most common checks in form of easy to use attributes. Wolfringo Commands System also allows easy implementation of own attributes for checks - see [Custom Requirements](Guides.Commands.Requirements#custom-requirements) to see how to create one.
